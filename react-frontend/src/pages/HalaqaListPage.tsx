...
interface HalaqaFormState {
  className: string;
  courseId: string;
}

const emptyForm = (): HalaqaFormState => ({ className: '', courseId: '' });
...
export default function HalaqaListPage() {
  ...
  const [form, setForm] = useState<HalaqaFormState>(emptyForm());
  ...
  const loadAll = async () => {
    try {
      setLoading(true);
      setError(null);
      const [crs, hal] = await Promise.all([courseApi.list(), halaqaApi.list()]);
      setCourses(crs);
      setHalaqas(hal);
    } catch {
  ...
  const openAdd = () => {
    setForm({ className: '', courseId: filterCourseId || (courses[0]?.id ?? '') });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingId === undefined) {
        const created = await halaqaApi.create(form);
        setHalaqas(prev => [...prev, created]);
      } else if (editingId) {
        const updated = await halaqaApi.update(editingId, { className: form.className });
        setHalaqas(prev => prev.map(h => h.id === editingId ? updated : h));
      }
      closeForm();
    } catch {
  ...
  const handleDelete = async (id: string) => {
    ...
    setDeletingId(id);
    try {
      await halaqaApi.delete(id);
      setHalaqas(prev => prev.filter(h => h.id !== id));
    } catch {
      alert(t.deleteError);
    } finally {
      setDeletingId(null);
  ...
  const filtered = filterCourseId
    ? halaqas.filter(h => h.courseId === filterCourseId)
    : halaqas;
    ...
    >
      <div className="max-w-4xl mx-auto px-4 py-8">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
          <div>
            <h2 className="text-2xl font-bold text-gray-900 dark:text-white">{t.halaqaManagement}</h2>
            {!loading && !error && (
              <p className="text-sm text-gray-500 dark:text-slate-400 mt-0.5">
                {filtered.length} {t.halaqas.toLowerCase()}
              </p>
            )}
          </div>
          {courses.length > 0 && (
        ...
        {isFormOpen && (
          <div className="bg-white dark:bg-slate-800 rounded-2xl border border-gray-200 dark:border-slate-700 p-6 mb-6 shadow-sm">
            <h3 className="text-base font-semibold text-gray-900 dark:text-white mb-4">
              {editingId === undefined ? t.addHalaqa : t.editHalaqa}
            </h3>
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center py-20 text-center">
            ...
            <p className="text-gray-500 dark:text-slate-400 mt-2 mb-7 text-sm max-w-xs">{t.noHalaqasDesc}</p>
          ...
          <div className="space-y-3">
            {filtered.map(h => {
              const crs = courses.find(c => c.id === h.courseId);
              return (
                <div
                  key={h.id}
                ...
                >
                  <div>
                    <h3 className="text-base font-semibold text-gray-900 dark:text-white">{h.className}</h3>
                    {crs && (
                      <p className="text-sm text-gray-500 dark:text-slate-400 mt-0.5">
                        📚 {crs.eventName}
                      </p>
