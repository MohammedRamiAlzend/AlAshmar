...
import { semesterApi } from '../api/academicApi';
import type { SemesterDto } from '../types/academic';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingId === undefined) {
        const created = await semesterApi.create(form);
        setSemesters(prev => [...prev, created]);
      } else if (editingId) {
        const updated = await semesterApi.update(editingId, form);
  ...
  return (
    ...
    >
      <div className="max-w-4xl mx-auto px-4 py-8">
        <div className="mb-6">
          <h2 className="text-2xl font-bold text-gray-900 dark:text-white">{t.semesterManagement}</h2>
          {!loading && !error && (
            <p className="text-sm text-gray-500 dark:text-slate-400 mt-0.5">
              {semesters.length} {t.semesters.toLowerCase()}
            </p>
          )}
        </div>
        ...
        {isFormOpen && (
          <div className="bg-white dark:bg-slate-800 rounded-2xl border border-gray-200 dark:border-slate-700 p-6 mb-6 shadow-sm">
            <h3 className="text-base font-semibold text-gray-900 dark:text-white mb-4">
              {editingId === undefined ? t.addSemester : t.editSemester}
            </h3>
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center py-20 text-center">
            ...
            <h2 className="text-xl font-semibold text-gray-800 dark:text-white">{t.noSemesters}</h2>
            <p className="text-gray-500 dark:text-slate-400 mt-2 mb-7 text-sm max-w-xs">{t.noSemestersDesc}</p>
